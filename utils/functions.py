...
def data_process(input_doc, relational_alphabet, tokenizer):
    ...
    for i in range(len(lines)):
        ...
        triplets = lines[i]["relationMentions"]
        target = {"relation": [], "head_start_index": [], "head_end_index": [], "tail_start_index": [], "tail_end_index": []}
        for triplet in triplets:
            head_entity = remove_accents(triplet["em1Text"])
            tail_entity = remove_accents(triplet["em2Text"])
            head_token = tokenizer.tokenize(head_entity)
            tail_token = tokenizer.tokenize(tail_entity)
            relation_id = relational_alphabet.get_index(triplet["label"])
...
def generate_span(start_logits, end_logits, info, args):
    ...
    for (start_prob, end_prob, seq_len, sent_idx) in zip(start_probs, end_probs, seq_lens, sent_idxes):
        output[sent_idx] = {}
        for triplet_id in range(args.num_generated_triplets):
            predictions = []
            start_indexes = _get_best_indexes(start_prob[triplet_id], args.n_best_size)
            end_indexes = _get_best_indexes(end_prob[triplet_id], args.n_best_size)
                ...
                for end_index in end_indexes:
                    ...
                    predictions.append(
                        _Prediction(
                            ...
                            end_index=end_index,
                            start_prob=start_prob[triplet_id][start_index],
                            end_prob=end_prob[triplet_id][end_index],
                        )
                    )
            output[sent_idx][triplet_id] = predictions
    return output
...
def generate_relation(pred_rel_logits, info, args):
    ...
    for (rel_prob, pred_rel, sent_idx) in zip(rel_probs, pred_rels, sent_idxes):
        output[sent_idx] = {}
        for triplet_id in range(args.num_generated_triplets):
            output[sent_idx][triplet_id] = _Prediction(
                            pred_rel=pred_rel[triplet_id],
                            rel_prob=rel_prob[triplet_id])
    return output


def generate_triplet(output, info, args, num_classes):
    ...
    for sent_idx in pred_rel_dict:
        triplets[sent_idx] = []
        for triplet_id in range(args.num_generated_triplets):
            pred_rel = pred_rel_dict[sent_idx][triplet_id]
            pred_head = pred_head_ent_dict[sent_idx][triplet_id]
            pred_tail = pred_tail_ent_dict[sent_idx][triplet_id]
            triplet = generate_strategy(pred_rel, pred_head, pred_tail, num_classes, _Pred_Triple)
            if triplet:
                triplets[sent_idx].append(triplet)
    # print(triplets)
    return triplets
