    ...
    def forward(self, outputs, targets):
        ...
        Params:
            outputs: This is a dict that contains at least these entries:
                 "pred_rel_logits": Tensor of dim [batch_size, num_generated_triplets, num_classes] with the classification logits
                 "{head, tail}_{start, end}_logits": Tensor of dim [batch_size, num_generated_triplets, seq_len] with the predicted index logits
        ...
        Returns:
            ...
            For each batch element, it holds:
                len(index_i) = len(index_j) = min(num_generated_triplets, num_gold_triplets)
        """
        bsz, num_generated_triplets = outputs["pred_rel_logits"].shape[:2]
        # We flatten to compute the cost matrices in a batch
        pred_rel = outputs["pred_rel_logits"].flatten(0, 1).softmax(-1)  # [bsz * num_generated_triplets, num_classes]
        gold_rel = torch.cat([v["relation"] for v in targets])
        # after masking the pad token
        pred_head_start = outputs["head_start_logits"].flatten(0, 1).softmax(-1)  # [bsz * num_generated_triplets, seq_len]
        ...
        cost = cost.view(bsz, num_generated_triplets, -1).cpu()
        num_gold_triplets = [len(v["relation"]) for v in targets]
